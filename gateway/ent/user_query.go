...
func (uq *UserQuery) loadAdmin(ctx context.Context, query *AdminQuery, nodes []*User, init func(*User), assign func(*User, *Admin)) error {
	...
	for _, n := range neighbors {
		fk := n.user_admin
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_admin" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) loadMeetings(ctx context.Context, query *MeetingQuery, nodes []*User, init func(*User), assign func(*User, *Meeting)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.MeetingsTable)
		s.Join(joinT).On(s.C(meeting.FieldID), joinT.C(user.MeetingsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.MeetingsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.MeetingsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "meetings" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
